...
function WorkspaceReview({ ws, section }: { ws: string; section: string }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    {section === "overview" ? <section className="border border-line bg-surface-2 p-5"><div className="flex flex-wrap items-center gap-3">
      ...
      {view?.demo && <p className="mt-2 text-sm">Judge challenge: inspect the duplicate candidate → open its source → assign an owner → add withheld service evidence → compare the new scan → export the briefing.</p>}</section> : <Link href="/scan" className="inline-block text-sm underline">Update results on the Scan page →</Link>}
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_1.1fr]"><div className="space-y-2">{filtered.map(f => <button key={f.id} onClick={() => { setSelected(f.id); setSource(null); }} aria-pressed={current?.id === f.id} className={`w-full border p-4 text-left ${current?.id === f.id ? "border-ink bg-surface-2" : "border-line"}`}>
          ...
          {current && <article className="self-start border border-line p-5"><h2 className="text-xl font-semibold">{current.title}</h2><p className="my-3 leading-relaxed">{current.explanation}</p><p className="text-sm text-ink-dim">{current.review}</p><h3 className="mt-5 font-semibold">Suggested next step</h3><p className="mt-1 text-sm">{current.action}</p>
            ...
            {source && <section aria-label="Original source" className="my-4 border border-line bg-surface-2 p-3"><b>{source.name}</b>
              {source.extraction_origin && <div className="my-2 text-sm"><p>This is a reviewed extraction, not the original document.</p><a className="underline" href={`${API_URL}/api/workspaces/${ws}/extraction/documents/${source.extraction_origin.document_id}/original`}>Download original: {source.extraction_origin.name}</a>{source.extraction_origin.has_images && source.extraction_origin.pages.map(p => <a key={p} target="_blank" rel="noreferrer" className="ml-3 underline" href={`${API_URL}/api/workspaces/${ws}/extraction/documents/${source.extraction_origin!.document_id}/pages/${p}`}>Original page {p}</a>)}</div>}
            ...
            {section === "actions" ? <FollowUpForm key={`${current.id}-${current.snapshot_id}-${current.follow_up?.version || 0}`} ws={ws} finding={current} saved={refresh} /> : <Link href="/approvals" className={control + " mt-5 inline-block"}>Assign an owner or record a decision →</Link>}
          </article>}</div>
      </>}
