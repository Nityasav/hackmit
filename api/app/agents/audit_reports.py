...
def report(ws: str, thread_id: str):
    with db.connect() as c:
        ...
        run_id = asked.get("run_id") or thread_id
        ...
        decisions = [dict(r) for r in c.execute("SELECT * FROM agent_decisions WHERE ws=? AND thread_id=? AND created_at>=? ORDER BY rowid", (ws, run_id, asked["created_at"]))]
        pending = c.execute("SELECT count(DISTINCT finding_id) FROM approvals WHERE ws=? AND run_id=? AND status='pending'", (ws, run_id)).fetchone()[0]
