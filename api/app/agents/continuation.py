...
async def resolve(ws, approval_id, decision):
    ...
    if graph:
        ...
        with db.connect() as connection:
            turn = connection.execute("SELECT id FROM conversations WHERE ws=? AND thread_id=? AND role='orchestrator' ORDER BY rowid DESC LIMIT 1",
                                      (ws, graph["run_id"])).fetchone()
            if turn:
                _finish(connection, ws, turn["id"], reply_for(outcome),
