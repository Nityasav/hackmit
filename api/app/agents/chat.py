...
def reply_for(run: dict) -> dict:
    ...
    if unresolved:
        lines.append(f"{len(unresolved)} scope or evidence gap(s) remain; see the details below.")
    if waiting:
...
async def talk(ws: str, body: Message):
    ...
    try:
        ...
        run = await run_investigation(ws, objective, thread_id=thread_id, meter=meter,
