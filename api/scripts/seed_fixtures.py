...
projection layer builds from the result.
    ...
    uv run python scripts/seed_fixtures.py --pack records.json          # offline, no cost
    uv run python scripts/seed_fixtures.py --pack records.json --live   # real model calls

The pack is supplied by whoever runs this, and the repository ships none: a
recording is only worth keeping if the records behind it are records someone
actually holds. It is one JSON object -- `start`, `end`, and a `files` list of
`{name, role, content}`, where a file marked `"later": true` is left out of the
commit -- and every number in the recording is computed from those bytes.
...
def commit_pack(name: str, pack: dict) -> str:
    """Create a workspace and commit every non-deferred file in the supplied pack."""
...
def main() -> int:
    ...
    parser.add_argument("--pack", required=True,
                        help="JSON pack of records to record over: {start, end, files[]}")
