...
system to be reconciled against. SchoolTrace takes inbound school money as first-class input:
...
them, receipts carrying no deposit reference at all, deposits that no supplied receipt accounts for, deposits
carrying no reference at all, receipts banked outside a five-day window that is a supplied demo convention
rather than a rule, the uncollected remainder of fee charges, collections naming a charge absent from the
register, and the unreceived remainder of pledges that fell due within the period. Each result is a
rules-based check over supplied records, not an Auditor verdict and not a loss. There is no bank feed,
payment processor or point-of-collection connector; refunds and payment plans are not modelled; a fee's
`waiver_reference` is stored but read by no check, so a waived fee is still reported as outstanding; and
money-in is not posted to the general ledger.

The dashboard no longer ships fixed demo workspaces; every screen reads a workspace somebody uploaded.
Connected five-agent investigation, invoice duplicate candidates, expense budget variance, payroll/grant
checks and human follow-up are implemented within the fictional profile. Full statements, structured
...
`SourcesPanel.tsx`. The separately merged `app/cfo/` coordinator provides a scripted multi-agent harness
and adapter interfaces over `/api/cfo/runs`; its own page has been removed, and the client that calls
those endpoints (`web/src/components/investigation/`) is not yet composed into `/investigation`. That
endpoint is distinct from the live snapshot-triage endpoint: live coordinator mode remains blocked until
specialist and auditor adapters are registered. Triage suggestions do not automatically dispatch those agents. `OPENAI_MODEL` configures
...
2. **Only a human decides.** Agents propose; `POST /api/workspaces/{ws}/review/actions` records the decision
   against a finding and a snapshot. No ledger, payment or grant submission is changed by it.
3. **Every agent action emits a `Decision`** (see `api/app/models.py`) so a reader can see what the agent
   did, why, and against what evidence, on `/investigation`.
